...
bool ImGui::ButtonEx(const char* label, ImColor colo, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Render
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ColoredButtonActive : hovered ? ImGuiCol_ColoredButtonHovered : ImGuiCol_ColoredButton);
...
bool ImGui::ColoredButton(const char* label, ImColor col, const ImVec2& size_arg)
{
...
bool ImGui::Button(const char* label, bool active, const ImVec2& size_arg)
{
    if (active)
        return ButtonEx(label, ImColor(200, 200, 200), size_arg, ImGuiButtonFlags_None);
    else
        return ButtonEx(label, size_arg, ImGuiButtonFlags_None);
}
