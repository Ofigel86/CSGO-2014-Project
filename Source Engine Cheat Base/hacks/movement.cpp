...
{
    if (!command || !g_variables || !g_context || !g_context->local_player)
        return;
...
void c_movement::auto_strafe(CUserCmd* command)
{
    ...
    __try {
        if (!(g_context->local_player->m_flags() & FL_ONGROUND) && command->m_sidemove == 0.0f && command->m_forwardmove == 0.0f)
        {
            static float fOldAbsoluteViewYaw = 0.0f;
...
void c_movement::fix_movement(CUserCmd* command)
{
    ...
    __try {
        ...
        if (v8 < 0.001f || v10 < 0.001f || v12 < 0.001f)
            return;
        ...
        if (v14 < 0.001f || v16 < 0.001f || v18 < 0.001f)
            return;
